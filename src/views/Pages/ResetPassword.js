    ...
    const response = resetPassword(payload)
            ...
            if (err.response) {
              if(err.response.data.message.email){
                return setError("Invalid email! Please request the reset code or try again!");
              }
              else if(err.response.data.message.token){
                return setError("Invalid token! Please request the reset code again!");
              }
                ...
                errvals['confirm_password'] = err.response.data.message.confirm_password
                seterrVals({ ...errvals })
              }
              else if(err.response.data.status == "passwords.token"){
                return setError("Invalid or expired token. Please request the reset code again!");
              }
              else if(err.response.data.status == "passwords.user"){
                return setError("Invalid email! Please request the reset code again!");
              }
              else{
        ...
        <InputRightElement width='4.5rem'>
          <Button h='1.75rem' size='sm' onClick={handlenewClick}>
            ...
            <Button
              ...
              onClick={() => { setError(undefined); seterrVals(errSet); setisloading(true); login()}}
            >
              {buttonText}
            </Button></Flex>
            </Flex>
